...
class DataPreprocessor:
    ...
    def prepare_data(self, features: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        try:
            ...
            X_scaled = self.scaler.fit(X)  # 학습 데이터로 스케일러 재학습
            x_train_scaled = self.scaler.transform(X_train)  # 테스트 데이터 변환
            X_test_scaled = self.scaler.transform(X_test)  # 테스트 데이터 변환
